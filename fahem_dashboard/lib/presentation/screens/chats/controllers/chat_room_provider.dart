...
class ChatRoomProvider with ChangeNotifier {
  ...
  Future<void> onPressedAddChatAndMessage({required BuildContext context, required String senderId, required String message, required ChatModel chat}) async {
    ...
    response1.fold((failure) async  {
      ...
      response2.fold((failure) async  {
        ...
        NotificationService.pushNotification(
          topic: '${chat.chatId}${ConstantsManager.fahemKeyword}',
          title: 'رسالة من فاهم', body: message,
        );
      });
    });
  }
