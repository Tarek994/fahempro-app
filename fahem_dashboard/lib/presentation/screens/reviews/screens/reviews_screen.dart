...
import 'package:fahem_dashboard/presentation/screens/reviews/widgets/review_list_item.dart';
...
class _ReviewsScreenState extends State<ReviewsScreen> {
  ...
  List<FiltersType> _getFiltersItems() {
    if(reviewsProvider.reviewsArgs == null) {
      return const [FiltersType.dateOfCreated, FiltersType.user, FiltersType.account];
    }
    else {
      if(reviewsProvider.reviewsArgs!.account != null) {
    ...
    return Consumer<ReviewsProvider>(
      builder: (context, provider, _) {
        return TemplateListScreen(
          ...
          goToInsertScreen: Methods.checkAdminPermission(AdminPermissions.addReview) ? () {
            Methods.routeTo(context, Routes.insertEditReviewScreen, arguments: null, then: (review) => _onInsert(review));
          } : null,
          title: _getTitle(),
