...
import 'package:fahem_dashboard/data/data_source/static/currencies_data.dart';
  ...
  static Future<CategoryModel?> categoriesBottomSheet({
    ...
    await Dialogs.showBottomSheet(
      ...
      child: SizedBox(
        height: MediaQuery.sizeOf(context).height * 0.8,
        child: FutureBuilder(
          future: DependencyInjection.getCategoriesUseCase.call(GetCategoriesParameters(filtersMap: filtersMap)),
          builder: (context, snapshot) {
            return WillPopScope(
              onWillPop: () => Future.value(snapshot.connectionState != ConnectionState.waiting),
              child: getWidget(snapshot),
